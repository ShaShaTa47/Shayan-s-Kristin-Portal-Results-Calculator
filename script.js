    ...
    document.forms['form3'].onsubmit = function () {
        ...
        chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
            chrome.tabs.sendMessage(tabs[0].id, { greeting: "class-names" }, function (response) {
                document.getElementById('thattextthing').innerHTML = "<strong>Please select on one or more subject(s) below, then press the 'select'button.<br>This functionality may only works on the 'find page' not on the standard results page!!! If it still doesn't work while you are on the correct page, refreshing your page should solve the issue. Otherwise contact the developer for support.</strong><br>";
                var checkboxstr="";
